...
public class CoursesAdapter extends RecyclerView.Adapter<CoursesAdapter.MyViewHolder> {
    ...
    private List<CourseEntity> coursEntity;
    private Context activityContext;
    private View.OnClickListener onItemClickListener;

    private static int colourIndex = 0;  // To scroll through colours

    public CoursesAdapter(List<CourseEntity> courseEntity, Context activityContext) {
        this.coursEntity = courseEntity;
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        final CourseEntity courseEntity = coursEntity.get(position);
        ...
        holder.letterGrade.setText(courseEntity.courseFinalGrade == null ? "N/A" : courseEntity.courseFinalGrade);
        ...
        Thread getTermThread = new Thread(() -> {
            ...
            Term courseTerm = termDao.getTermById(courseEntity.courseTermId);

            holder.courseRowCode.setText(String.format(Locale.CANADA, "%s %s",
                    courseTerm.asShortString(), courseEntity.courseCode));
        });
        getTermThread.start();
        try {
            getTermThread.join();
